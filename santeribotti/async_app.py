...
async def message(**payload):
    ...
    if text and text.lower() == "mee nukkumaan":
        print("stoppista")
...
def GPIOsetup():
    ...
    GPIO.setup(BUTTON_PIN, GPIO.IN) # adjust the PULL UP/PULL DOWN as applicable
    GPIO.add_event_detect(BUTTON_PIN, GPIO.RISING, callback=lambda x: motion_sensor(message_manager_f), bouncetime=2000)

def message_manager_f():
    print(":P message_manager_f()")

def motion_sensor(message_manager_f):
    send_message("oispa kahvia")
    if loop is None: # should not come to this
        print("Loop error")
        return
...
def shutdown():
    ...
    for task in pending:
        task.cancel()
        # TODO tulee error koska rtm_client ei oo valmis...
